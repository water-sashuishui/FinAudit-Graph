...
- **Chroma 向量数据库 RAG**：使用 `data/rag/audit_standards.json` 构建 `data/rag/chroma_db/`，优先按向量相似度检索审计准则片段；异常时回退关键词检索。
...
说明：当前 MVP 已使用 `Chroma` 作为本地持久化向量数据库，并继续采用 `local_hashing_v1` 本地文本向量方式，优点是无需下载 embedding 模型、无需联网、答辩现场稳定可复现。后续生产化可以将 embedding 层替换为更强的 sentence-transformers 或云端 embedding 服务。
...
6. 讲解 Neo4j fallback 与 Chroma 向量数据库 RAG 如何保证演示稳定，以及后续如何替换为真实服务。
...
- 将当前 Chroma 向量库接入更强的 embedding 模型，或按生产需求切换到 FAISS / Milvus。
